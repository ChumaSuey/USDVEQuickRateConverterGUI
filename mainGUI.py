...
class BcvDashboardApp:
    def __init__(self, root):
        ...
        self.root.bind("<Control-r>", lambda e: self.load_api_data_first_time())
        self.root.bind("<Control-R>", lambda e: self.load_api_data_first_time())
        self.root.bind("<Control-e>", lambda e: self.export_data())
        self.root.bind("<Control-E>", lambda e: self.export_data())
        self.root.bind("<Control-c>", self.copy_selected_rate)
        self.root.bind("<Control-C>", self.copy_selected_rate)
    ...
    def create_filter_bar(self):
        filter_frame = tk.LabelFrame(self.root, text=" Filtros y Calendario Continuo ", font=("Segoe UI", 9, "bold"),
                                     fg="#bac2de", bg="#1e1e2e", bd=1, relief="groove")
        ...
        years_list = [str(y) for y in range(now.year - 1, now.year + 2)]
        self.combo_year = ttk.Combobox(filter_frame, values=years_list, width=6, state="readonly")
        self.combo_year.pack(side="left", padx=2)

        self.combo_month.set(months_list[now.month - 1])
        self.combo_year.set(str(now.year))

        self._ui_buttons["filter"] = tk.Button(
        ...
        self._ui_buttons["filter"].pack(side="left", padx=8)

        self._ui_buttons["hoy"] = tk.Button(
            filter_frame, text="Hoy", font=("Segoe UI", 9, "bold"),
            bg="#a6e3a1", fg="#11111b", activebackground="#94e2d5", bd=0, padx=8, cursor="hand2",
            command=self.jump_to_current_month
        )
        self._ui_buttons["hoy"].pack(side="left", padx=2)

        self._ui_buttons["reset"] = tk.Button(
        ...
        self._ui_buttons["reset"].pack(side="right", padx=5)
    ...
    def create_footer_controls(self):
        ...
        self._ui_buttons["refresh"] = tk.Button(
            footer, text="Recargar API  (Ctrl+R)", font=("Segoe UI", 10, "bold"),
            bg="#313244", fg="#cdd6f4", bd=0, padx=15, pady=8, cursor="hand2",
            command=self.load_api_data_first_time
        )
        self._ui_buttons["refresh"].pack(side="left")

        self._ui_buttons["export"] = tk.Button(
            footer, text="Exportar Reporte  (Ctrl+E)", font=("Segoe UI", 10, "bold"),
            bg="#a6e3a1", fg="#11111b", activebackground="#94e2d5", bd=0, padx=20, pady=8, cursor="hand2",
            command=self.export_data
        )
        self._ui_buttons["export"].pack(side="right")
    ...
    def copy_rate_to_clipboard(self, event):
        ...
        self.lbl_status_date.config(text=f"Copiado: {clean_rate}!", fg="#a6e3a1")
        self.root.after(2000, lambda: self.lbl_status_date.config(text=old_text, fg="#a6adc8"))

    def copy_selected_rate(self, event=None):
        self.copy_rate_to_clipboard(event)
    ...
    def _set_ui_loading(self, loading):
        ...
        for key, btn in self._ui_buttons.items():
            btn.config(state=state)
        for combo in [self.combo_currency, self.combo_month, self.combo_year]:
            combo.config(state="disabled" if loading else "readonly")
    ...
    def load_api_data_first_time(self):
        ...
        self.lbl_status_date.config(text="Conectando con el BCV...", fg="#f9e2af")

        def _fetch():
            records = fetch_all_bcv_history(currency=self.current_currency)
            self.root.after(0, lambda: self._on_data_loaded(records))
    ...
    def _on_data_loaded(self, records):
        ...
        self.all_historical_records = records

        if not self.all_historical_records:
            messagebox.showerror(
                "Sin Conexión a Internet",
                "No se pudo conectar con el servidor de tasas.\n\n"
        ...
        self.apply_current_month_from_filter()

    def apply_current_month_from_filter(self):
        if not self.all_historical_records:
        ...
        if filtered:
            ...
            self.lbl_status_date.config(
                text=f"Sin datos para {selected_month_name} {selected_year}. Mostrando últimos 30 días.", fg="#f9e2af"
            )
    ...
    def jump_to_current_month(self):
        now = datetime.datetime.now()
        months_list = list(self.months_map.keys())
        self.combo_month.set(months_list[now.month - 1])
        self.combo_year.set(str(now.year))
        self.apply_monthly_filter()
        ...
        for r in reversed(self.current_view_records):
            if r["is_filled"]:
                self.tree.insert("", "end", values=(
                    r["date"], f"{format_currency_ve(r['rate'])} Bs. (Feriado/FDS)"
                ), tags=("filled",))
            else:
                self.tree.insert("", "end", values=(
                    r["date"], f"{format_currency_ve(r['rate'])} Bs."
                ))
    ...
    def export_to_xlsx(self):
        ...
        default_name = (
            f"Calendario_Continuo_BCV_{cur_code}_{self.selected_month_year_str}.xlsx"
            if self.is_filtered_view
            else f"Calendario_Continuo_BCV_{cur_code}_30_Dias.xlsx"
        )
        ...
        try:
            ...
            for record in self.current_view_records:
                ...
                cell_rate.alignment = right_align
                cell_rate.border = thin_border
                cell_rate.number_format = '#,##0.00'

                if record["is_filled"]:
                    cell_date.font = filled_font
                    cell_date.fill = filled_row_fill
                    cell_rate.font = filled_font
                    cell_rate.fill = filled_row_fill
                else:
    ...
    def export_to_csv(self):
        cur_code = "USD" if getattr(self, "current_currency", "dolares") == "dolares" else "EUR"
        default_name = (
            f"Calendario_BCV_{cur_code}_{self.selected_month_year_str}.csv"
            if self.is_filtered_view
            else f"Calendario_BCV_{cur_code}_30_Dias.csv"
        )
        file_path = filedialog.asksaveasfilename(
            defaultextension=".csv", filetypes=[("Archivos CSV", "*.csv")],
            title="Guardar tasas (CSV)", initialfile=default_name
        )
        if not file_path:
            return
        try:
